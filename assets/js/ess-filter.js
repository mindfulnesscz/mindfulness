...
function initialize_filter_object(holder_id, row_class, item_class) {
  var holder_el = document.getElementById(holder_id);
  var rows = holder_el.querySelectorAll('.' + row_class);

  //console.log(rows);
  var _loop = function _loop() {
    var row = rows[n];
    var row_name = row.getAttribute('row');
    FilterObject[row_name] = new Object();
    var items = row.querySelectorAll('.' + item_class);
    var _loop2 = function _loop2() {
      var item = items[i];
      var item_name = item.getAttribute('data-filter');
      var item_link = item.querySelector('button');
      ...
      item_link.addEventListener('click', function () {
        ...
        if (item_link.classList.contains('filter-active')) {
          ...
          if (item.getAttribute('data-filter') == 'all') {
            for (var _n in FilterObject[row_name]) {
              if (_n != 'all') {
                FilterObject[row_name][_n]['filter-active'] = false;
                console.log(FilterObject[row_name][_n]['filter-element'].querySelector('button').classList);
                FilterObject[row_name][_n]['filter-element'].querySelector('button').classList.remove('filter-active');
              }
            }
        ...
        // else if the item is being de-activated
          ...
          FilterObject[row_name][item_name]['filter-active'] = false;
          var row_all_passive = true;
          for (var o in FilterObject[row_name]) {
            if (FilterObject[row_name][o]['filter-active'] == true) row_all_passive = false;
          }
          if (row_all_passive) {
      ...
      if (item_link.classList.contains('filter-active')) FilterObject[row_name][item_name]['filter-active'] = true;else FilterObject[row_name][item_name]['filter-active'] = false;
    };
...
function build_isotope_query() {
  ...
  for (var key in FilterObject) {
    if (!FilterObject[key]['all']['filter-active']) {
      // if all is not active

      var row = [];
      for (var item_key in FilterObject[key]) {
        if (FilterObject[key][item_key]['filter-active'] == true) row.push(item_key);
      }
      if (row.length != 0) FilterArray.push(row);
    }
  }

  //removes rows without any filter item active	
  for (var i = 0; i < FilterArray.length; i++) {
    if (FilterArray[i].length == 0) FilterArray.splice(i, 1);
  }
  if (FilterArray.length != 0) {
    //if there is any filter row item activated at all..
...
function filter_izer() {
  var $grid = jQuery('.filtr-container');
  var $items = $grid.find('.filtr-item');
  console.log('items ');
  console.log($items);
  $items.each(function () {
    var link = jQuery(this).find('.image-link');
    var w = jQuery(link).width();
    console.log('--------------');
    console.log(link);
    console.log(w);
...
/******/ })()
;
//# sourceMappingURL=ess-filter.js.map