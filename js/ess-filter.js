/*jshint esversion: 6 */
//console.log('ahooooooooooj filterrrr');


document.addEventListener("DOMContentLoaded", function(){
	//console.log('woooot');
	//console.log(document.Filterizr);
...
/*function filter_recursive_loop(query_string, row, col){
	
	let arr = FilterArray[row];
	if(arr){
		
		//1. update query string for isotope filter
		
		col_string = FilterArray[row][col];
		query_string += '.'+col_string;

		//2. itterate numbers for next go

		if(row+1 == FilterArray.length){// end of the line
			row=0;
		}
		...
		/*
		col ++;
		if (col == FilterArray[row].length){
			col = 0;
			row ++;
			if (row == FilterArray.length)
				return query_string; // THIS IS THE FINAL END OF THE RECURSION if this is tha last row and there is no else to go
		}	

		//3. trigger next iteration
		
		console.log('row a col jsou '+row+', '+col);

		query_string = filter_recursive_loop(query_string, row, col);
		return query_string;
