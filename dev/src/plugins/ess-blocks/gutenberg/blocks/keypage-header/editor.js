...
const { MediaUpload,  InspectorControls, PanelColorSettings, InnerBlocks, withColors, getColorClassName } = wp.blockEditor;
...
const block = registerBlockType('mindfulness-blocks/keypage-header', {
    ...
    edit: withColors( 'backgroundColor' )( props => {
        ...
        const { setAttributes, attributes, backgroundColor, setBackgroundColor } = props;
        const { headerType, illustration } = attributes;

        let headerClasses = ( 'mindfulness-wp-block ' + headerType + ' ' + ( backgroundColor.class || '' ) ).trim();
        ...
        function selectImage ( imageObject) {
            ...
            let img_arr = EssImageSrcset.ess_get_image_srcset(imageObject.sizes);

            let sizes = "(min-width: 760px) calc((100vw/2.5)*"+ img_arr[2] +"), (min-width: 561px) calc(100vw/2), 280px";
            
            setAttributes ({
                illustration_arr: EssImageSrcset.ess_get_image_srcset ( imageObject.sizes ),
        ...
        <header id="ess-header" className = { headerClasses } >
             <div class="ess-keypage-header">
                    ...
                    <img src={(function(){ return illustration;})()}></img>
    ...
    save ( props ) {
        ...
        const { illustration, illustration_srcset, illustration_arr, illustration_sizes, headerType, backgroundColor } = attributes;
        
        let headerClasses = ( 'mindfulness-wp-block ' + headerType + ' ' + ( getColorClassName ('background-color', backgroundColor ) || '' )).trim();
        ...
        return (
            <header id = "ess-header" className = { headerClasses } >
                <div class = "ess-keypage-header" >
                    ...
                    <img src={illustration} srcset={illustration_srcset} sizes={illustration_sizes}></img>
