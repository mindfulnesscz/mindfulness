...
function resize_column_parent(id,  values){
    ...
    if (div){
        ...
        //apply_class(div, values);
    }
    else {
        setTimeout(()=>{resize_column_parent(id, values);},100);
    }

}
function apply_class(element, values){
    element.classList.add('col-xs-'+values[0], 'col-sm-'+values[1] , 'col-md-'+values[2], 'col-lg-'+values[3]);
    console.log(element);
}
...
const block = registerBlockType('mindfulness-blocks/flexbox-col', {
    ...
    edit(props) {
        ...
        const { col_class, col_xs, col_sm, col_md, col_lg } = attributes;
