...
window.ess_index = function () {
    ...
    let el_nav_button = document.getElementById('ess-menu-toggle');
    let el_sidemenu = document.getElementById('ess-side-menu');
    let el_intro = document.getElementById('ess-intro');
    let el_cubenavholder = document.getElementById('ess-cube-navigation-holder');

    let CSSCube = new CessCube(el_cubenavholder);
    CSSCube.init();
    CSSCube.setup_cube();

    let EssToggler = new NavbarToggle(el_nav_button, el_sidemenu, CSSCube, el_intro, window.ishomepage);
    EssToggler.init();
    ...
    let home_banner = new FooterSlider ( document.getElementById ( 'ess-home-banner' ), { autoplay: true, center_arrows: true } );
    home_banner.init();
    ...
    var ess_modals = document.querySelectorAll('.modal');
    var ess_modal_instances = M.Modal.init(ess_modals, {
        opacity: 0.2
    });
...
function activate_bottom_cube(EssToggler) {
    let c = document.getElementById('bottom-cube');
    if (c) {
...
function make_cube() {
    ...
    let base_path = '';
    if (window.template_url) // generated in wordpress
    ...
    if (window.ess_url_array)
        url_array = window.ess_url_array;
    ...
    //Cube
    let Cube = new EssCube(window.ishomepage, container, canvas, main_container);
...
function activate_sidebar_contact() {
    ...
    let el_sidebar_contact = document.getElementById('ess-side-contact');
    if (el_sidebar_contact) {

        let main_el = document.querySelector('main');
        let toggler = document.createElement('div');
        let toggler_text = document.createElement('div');
        let toggler_text_h = document.createElement('h4');
        let toggler_text_h_content = document.createTextNode('GET IN TOUCH');
        let toggler_text_span = document.createElement('span');
        ...
        let el_sidebar_contact_toggler = toggler;
        let el_contact_sales_toggler = document.getElementById('ess-contact-sales-button');
        ...
        if (el_sidebar_contact_toggler || el_contact_sales_toggler) {
            ...
            // Materialize sidebar call - DEPRACATED NOW USING ONLY CLICK LINK
            /*
            M.Sidenav.init(el_sidebar_contact, {
                edge: 'right'
            });
            var menu_inst = M.Sidenav.getInstance(el_sidebar_contact);
...
function stick_contact_button() {
    let contact_button = window.el_sidebar_contact_toggler;
    let header_h = window.el_header.offsetHeight;
    let fixed_top = (window.vh - el_sidebar_contact_toggler.offsetHeight) / 2;

    if (header_h > fixed_top) {
        if (window.sc >= header_h - fixed_top && window.contact_fixed == false) {
            contact_button.classList.add('contact-fixed');
            contact_button.style.top = fixed_top + 'px';
            window.contact_fixed = true;

        } else if (window.sc <= header_h - fixed_top && window.contact_fixed == true) {
...
function hover_people() {
    let people_array = document.querySelectorAll('.ess-hover-block');
    if (people_array.length != 0) {
        console.log('people array is ' + people_array.length);
        for (let key in people_array) {
            let item = people_array[key];
            let toshow = item.querySelector('.ess-hover-toshow');
        ...
        console.log('people_array is empty');
}